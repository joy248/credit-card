...
import { getTopCreditCards } from "@/lib/credit-cards"

export default async function FeaturedCards() {
  const featuredCards = await getTopCreditCards()
          ...
          <CardContent className="pb-2">
            <div className="space-y-4">
              <div className="h-40 bg-gradient-to-r from-gray-100 to-gray-200 rounded-lg flex items-center justify-center">
                <div className="w-3/4 h-44 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl transform rotate-6 shadow-lg flex items-center justify-center text-white font-medium">
                  {card.bank} {card.name}
                </div>
              </div>
